...
! 2019/08/03 use crust1.0 velocity to do location
program main
...
real :: vb,ve,stp_v,vv,tt(maxnsta),sto,sta,v_ref(maxnsta),vtemp
integer :: iv,nstpv,tp
...
write(refvfile,'("output_gv_",i2.2,".dat")')tp
write(*,*)'reference file is', refvfile
inquire(file=refvfile,exist=ext)
if(.not.ext)stop 'Reference velocity not found.'
is=1
...
open(14,file=refvfile)
do is=1,maxnsta
   read(14,*,end=15,err=15)tmp1,tmp,vtemp
   do i=1,nsta
      if(tmp.eq.station(i))v_ref(i)=vtemp
   enddo
enddo
15 close(14)
! Begin grid search
...
do i=1,nstp_lo                                                       ! loop over lontitude
   ...
   do j=1,nstp_la                                                    ! loop over lantitude
      ...
      do is=1,nsta                                                   ! loop over tele stations
         ...
         if(tt(is).ne.0)&
         t00=t00+tt(is)-dist(is)/v_ref(is)
      enddo                                                          ! end loop over stations
      t00=t00/imark                                                  ! the original time
      error_all=0;errorabs=0
      do is=1,nsta                                                   ! loop over tele stations
         if(tt(is).ne.0)then
            error_all=error_all+(tt(is)-dist(is)/v_ref(is)-t00)**2/imark     !error1=error1+abs(dist/tt-v)**2
            errorabs =errorabs+ abs(tt(is)-dist(is)/v_ref(is)-t00)/imark
         endif
      enddo                                                          ! end loop over stations
      write(21,'(7f15.5)')stloo,stlaa,sqrt(error_all),errorabs,t00
